...
namespace GameMain
{
    ...
    /// 配置组件。
    /// </summary>
    public  class SettingComponent : Singleton<SettingComponent>
    {
        private ISettingManager m_SettingManager = null;

        public ISettingManager ISettingManager
        {
            ...
            get
            {
                ...
                {
                    m_SettingManager = GameMainEntry.GetModule<SettingManager>();
        ...
        [SerializeField]
        private ISettingHelper m_CustomSettingHelper = null;





        public  readonly string Str_Setting = "/SettingData";
        ...
        public SettingData SettingData
        {
            get
            {
                if (m_settingdata ==null)
                {
                    ...
                    string path = GetSavePath(Str_Setting);
                    if (File.Exists(path))
                    {
                        m_settingdata = FileTool.GetDataFromFile<SettingData>(path);
                    }
                    else
                    {
                        m_settingdata = new SettingData();
                        FileTool.SaveDataToFile<SettingData>(path, m_settingdata);
                    }
...
#endif
            string savepath = mDevicePersistentPath +strpath;
            return savepath;
        }
